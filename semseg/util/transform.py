...
from collections.abc import Iterable  # 导入Iterable 替换 collections.Iterable 为 collections.abc.Iterable
...
class RandShift(object):
    # Randomly shift image & label with shift factor in [shift_min, shift_max]
    def __init__(self, shift, padding, ignore_label=0, p=0.5):
        assert (isinstance(shift, Iterable) and len(shift) == 2)
        if isinstance(shift[0], numbers.Number) and isinstance(shift[1], numbers.Number) and shift[0] <= shift[1]:
            self.shift = shift
        else:
            raise (RuntimeError("segtransform.RandShift() shift param error.\n"))
        assert padding is not None
        assert isinstance(padding, list) and len(padding) == 3
        if all(isinstance(i, numbers.Number) for i in padding):
            self.padding = padding
        else:
            raise (RuntimeError("padding in RandShift() should be a number list\n"))
        assert isinstance(ignore_label, int)
        self.ignore_label = ignore_label
        self.p = p
    ...
    def __call__(self, image, label):
        if random.random() < self.p:
            shift_x = random.uniform(self.shift[0], self.shift[1])
            shift_y = random.uniform(self.shift[0], self.shift[1])
            h, w = label.shape
...
class RandColor(object):
    # Randomly change image & label with brightness,contrast factor in [brightness_min, brightness_max], [contrast_min, contrast_max]
    def __init__(self, brightness, contrast):
        if isinstance(brightness, numbers.Number) and 0 < brightness < 1:
            self.brightness = brightness
        else:
            raise (RuntimeError("segtransform.RandColor() brightness param error.\n"))

        if isinstance(contrast, numbers.Number) and 0 < contrast < 1:
            self.contrast = contrast
        else:
            raise (RuntimeError("segtransform.RandColor() contrast param error.\n"))

    def __call__(self, image, label):
        # Randomly select the ratio of brightness and contrast changes
        ...
        mean = np.mean(image)
        # print("===========/****",mean)
        # print(mean*(1-contrast_factor))
        image = cv2.convertScaleAbs(image, alpha=contrast_factor, beta=mean*(1-contrast_factor))

        # no label random brightness and contrast changes
        return image, label
