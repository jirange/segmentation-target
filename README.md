...
切实可用的命令
sc create --name svd-simple-task --image "harbor.smoa.cc/public/xsemseg:v1.4" --gpu 2 --cmd " source /dataset/vsitongwu/.bashrc && cd /dataset/vsitongwu/LJC/svd/simple && conda activate svd-simple && python ./simple.py " --debug --arch ampere
...
(animate) [vsitongwu@login-01-04 semseg]$ sc create --name seg  --image "harbor.smoa.cc/public/xsemseg:v1.4" --gpu 2 --cmd "source /dataset/vsitongwu/.bashrc && cd /dataset/vsitongwu/LJC/TargetSeg/semseg && conda activate animate && sh tool/mytrain.sh target pspnet50" --arch ampere --node node66
...
应该使用 collections.abc.Iterable 而不是 collections.Iterable。collections.abc 是在Python 3中引入的，用于提供抽象基类（ABCs）
...
tool/mytrain.sh target pspnet50


git remote add origin git@github.com:jirange/TargetSeg.git
git branch -M main
git push -u origin main

(animate) [vsitongwu@login-01-04 TargetSeg]$ git push -u origin master
Warning: Permanently added the ECDSA host key for IP address '20.205.243.166' to the list of known hosts.
ERROR: Permission to jirange/TargetSeg.git denied to tianzhuotao.
...
train_h train_w引起的   或者是transform ignore_label 随意变换引起的  
...
maybe transform  yes not randshift  not randcolor is normal   ---> yes randshift no randcolor  is normal
